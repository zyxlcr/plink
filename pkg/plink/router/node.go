...
func (n *node) add(path string, handler iface.HandlerFunc) (lp uint8) {
	var err error
	if path == Blank {
		path = BasePath
	}
...
func (n *node) insertChild(numParams uint8, existing existingParams, path string, fullPath string, handler iface.HandlerFunc) {
...
func (n *node) Find(path string, mux *Router) (handler iface.HandlerFunc, rv *requestVars) {
