...
func (m *BaseMapper[T]) Insert(data any) error {
	err := m.GVA_DB.Model(m.t).Create(data).Error
	return err
}

func (m *BaseMapper[T]) Update(column string, value any) error {
	return m.GVA_DB.Model(m.t).Update(column, value).Error
}

func (m *BaseMapper[T]) Updates(value T) error {
	return m.GVA_DB.Model(m.t).Updates(value).Error
}

func (m *BaseMapper[T]) UpdatesById(id int64, value any) error {
	return m.GVA_DB.Model(m.t).Select("*").
		Omit("created_at", "id").
		Where("id = ?", id).
		Updates(value).Error
}
...
func (m *BaseMapper[T]) DeleteById(id int64) error {
	err := m.GVA_DB.Model(m.t).Delete("id", id).Error
	return err
}

func (m *BaseMapper[T]) SelectById(id int64) error {
	err := m.GVA_DB.Model(m.t).First(m.t, id).Error
	return err
}

func (m *BaseMapper[T]) GetById(id int64) (T, error) {
	var val T
	err := m.GVA_DB.Model(m.t).First(&val, id).Error
	return val, err
}
...
func (m *BaseMapper[T]) QueryCount() int64 {
	var count int64
	db := m.GVA_DB.Model(m.t).Where(m.t)
	err := db.Count(&count).Error
	if err != nil {
		panic(err)
	}
...
func NewMapper[T Model](t T, w *wrapper) Mapper[T] {
	return &BaseMapper[T]{t: t, w: w, GVA_DB: config.GVA_DB}
}
